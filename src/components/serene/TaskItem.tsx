...
export const TaskItem = forwardRef<HTMLDivElement, TaskItemProps>(
  ({ task, onToggle, onDelete, onUpdate, isDragging, ...props }, ref) => {
    ...
    if (isMobile) {
      return (
        <div ref={ref} {...props} className="w-full origin-top">
          <div className={cn(
    ...
    const cardShouldExpand = !isEditing && isLongDescription;
    ...
    return (
      <div
        ...
        className={cn(
          "w-full origin-top",
          cardShouldExpand && "md:col-span-2 xl:col-span-2",
        )}
      >
        <Card
          className={cn(
            'bg-card/80 backdrop-blur-sm border-border/50 shadow-pop flex flex-col w-full transition-shadow duration-200',
            cardShouldExpand ? 'h-full' : 'h-64',
            task.completed && 'bg-card/60',
            isDragging && 'opacity-80 shadow-2xl'
          )}
